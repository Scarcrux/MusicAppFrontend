...
class RecommendateEvents(Resource):
    @classmethod
    def get(cls,user_id):
        address_id = UserModel.find_by_id(user_id).address_id
        zip = AddressModel.find_by_id(address_id).zip
        addresses = AddressModel.query.filter_by(zip=zip).all()
        events = []
        for address in addresses:
            event  = EventModel.query.filter_by(address_id=address.id).first()
            if event and event.date>datetime.datetime.now() and not event.user_attended(user_id):
                events.append(event)
